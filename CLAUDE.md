...
- **Video** (`app/components/media/LoopVideo.tsx`, `app/hooks/useVideoPlayback.ts`) — never autoplays by attribute: one "play group" per scroller plays, the one under a reading line that walks down the scroller, so on load only the first cover plays. There is no pause control (Nic's call): the active group loops continuously, and only the system's reduced-motion setting leaves the stills. Files are in `public/work/<slug>/<slot>.{mp4,jpg}`.
